...
from huawei_commands.delete_profile import DeleteProfile
from huawei_commands.lcman_disable_slot import LcmanDisableSlot
from huawei_commands.lcman_reset_slot import LcmanResetSlot
from huawei_commands.lcman_enable_slot import LcmanEnableSlot
from huawei_commands.port_pvc_set import PortPvcSet
...
from huawei_commands.sys_snmp_setcommunity import SysSnmpSetCommunity
from huawei_commands.sys_snmp_getcommunity import SysSnmpGetCommunity
from huawei_commands.show_lineinfo import ShowLineInfo
from huawei_commands.show_linestat_slot import ShowLineStatSlot
from huawei_commands.show_performance import ShowPerformance
'''
    ...
    def get_port_index_mapping(cls, dslam_data):
        ...
        try:
            ...
            for item in var_binds:
                if 'adsl' in (item.value).lower():
                    slot_number = item.value.split('/')[1]
                    port_number = item.value.split('/')[2]
                ...
                if type_dslam_port == 'slot':
                    for slot in slot_port_objs:
                        for command in commands:
                            template_tag = command.get('command_template')
