...
class PortmanRPC(object):
    ...
    def __init__(self, portman_runner):
        ...
        self.portman = Portman(django_orm_queue=self.portman_runner.django_orm_queue, request_q=self.portman_runner.request_q, zyxel_q=self.portman_runner.zyxel_q, zyxel1248_q=self.portman_runner.zyxel1248_q, fiberhomeAN2200_q = self.portman_runner.fiberhomeAN2200_q, fiberhomeAN5006_q=self.portman_runner.fiberhomeAN5006_q, fiberhomeAN3300_q=self.portman_runner.fiberhomeAN3300_q)
...
class Portman_Runner(object):
    ...
    def add_dslam_port_busy(self, dslam_id, busy_key, command):
        item = str(dslam_id)+'-'+busy_key+'-'+command
    ...
    def create_process(self):
        ...
        worker = Worker(dslam_port_busy_queue=self.dslam_port_busy_queue, queue=self.service_queue, django_orm_queue=self.service_django_orm_queue, \
                request_q=self.request_q, zyxel_q=self.zyxel_q, fiberhomeAN2200_q=self.fiberhomeAN2200_q, fiberhomeAN5006_q=self.fiberhomeAN5006_q, fiberhomeAN3300_q=self.fiberhomeAN3300_q)
        worker.start()
    ...
    def resync_dslams(self):
        ...
        for index, dslam in enumerate(queryset, 1):
            ...
            #if dslam.id not in (399,818,921,783,785,168,870,781,288,267,976,182,678,674,796,677,711,977,903,624,721,714,895,713,549,722,234,640,163,806,220,951,231,378,247,236,863,893,672,728,716,892,222,808,437,891,811,630,170,710,185,207,171,864,202,204,573,166,621,814,809,381,397,454,213,289,453,902,380,169,188,377,918,736,294,529,805,987,919,628,225,162,206,191,208,192,198,210,183,367,491,313,315,983,981,639,949,984,508,487,954,901,898,894,329,971,1086,365,366,665,675,725,318,189,719,922,812,249,670,379,593,252,862,681,667,493,816,673,224,400,712,490,500,717,985,221,295,952,314,900) or dslam.status == 'error':
            #if dslam.id not in (399,) or dslam.status == 'error':
                continue
