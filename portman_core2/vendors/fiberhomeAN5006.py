...
class FiberhomeAN5006(BaseDSLAM):
        ...
        try:
            ...
            for item in var_binds:
                if 'adsl' in item.value.lower():
                    slot_number, port_number = map(unicode.strip, item.value.split()[1].split('/'))
    ...
    def get_port_vpi_vci(cls, dslam_info):
        ...
        try:
            ...
            info['port_vpi_vci'] = port_vpi_vci.values()
            return info
    ...
    def execute_command(cls, dslam_info, command, params):
        ...
        print '---------------------------'
        print dslam_info.get('id'), command, params
        print '---------------------------'
        return c.telnet_run_command(dslam_info.get('id'), command, params)
