...
class DisplayIfSipAttributeRunning(BaseCommand):
    ...
    def run_command(self):
        try:
            ...
            print('display if-sip attribute running')
            tn.write(b"\r\n")
            tn.write(b"\r\n")
            tn.write(b"\r\n")
            tn.write(b"\r\n")
            ...
            tn.write(b"y\r\n")
            tn.close()
            return dict(result=re.sub(r'-{2,}\sMore\s\(\s\w+\s\'\w\'\s\w+\s\w+\s\)\s-{2,}\\\w\d+\w\[\d+\w|\\\w\d+\w\[\d+\w', result).decode('utf-8'), status=200)
