...
class SetProfile(BaseCommand):
    ...
    def run_command(self):
        try:
            ...
            tn.write("adsl profile map {0} {1} auto\r\n\r\n".format(self.__port_indexes[0]['port_number'],
                                                                    self.__lineprofile).encode('utf-8'))
            tn.write("end\r\n")
            res = tn.read_until("end")
            if ('does not exist' in res):
                result = 'Profile {0} does not exist.'.format(self.__lineprofile)
            else:
                result = 'Port Profile has been changed to {0}'.format(self.__lineprofile)
            tn.write("exit\r\n")
            tn.write("y\r\n")
            tn.close()
            print('*******************************************')
