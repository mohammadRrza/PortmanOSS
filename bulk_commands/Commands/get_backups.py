        ...
        try:
            ...
            for SwitchObj in SwitchObjs:
                try:
                    ...
                    f = open("/home/mrtbadboy/backup/cisco_switches/{0}_{1}.txt".format(
                    ...
                    f = open("/home/mrtbadboy/backup/cisco_switches/Error_{0}_{1}.txt".format(
                        SwitchObj.device_fqdn, str(datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S'))), "w")
            ...
            for RouterObj in RouterObjs:
                try:
                    ...
                    f = open("/home/mrtbadboy/backup/mikrotik_routers/{0}_{1}.txt".format(
                    ...
                    print(str(ex)+" "+"54")
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    f = open("/home/mrtbadboy/mikrotik_routers/backup/Error_{0}_{1}.txt".format(
                        RouterObj.device_fqdn, str(datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S'))), "w")
                    f.write(str(ex) + "  // " + str(exc_tb.tb_lineno))
                    f.close()
                    client.close()
