        ...
        try:
            ...
            for line in lines:
                ...
                if status == '-':
                    ...
                    time.sleep(2)
                    tn.write("end{0}\r\n".format(card_number))
                    result = tn.read_until("end{0}".format(card_number))
                    try:
                        serial_number = re.search(r'hardware serial number:\s(?:\w+)?',result , re.M).group(0).split(':')[1].strip()
                        temperature = [{'name':temp.split(':')[0].strip(), 'value':temp.split(':')[1].strip()} for temp in re.findall(r'Temp\d+:\s\d+\.\d+',result , re.M)]
                        hw_version = re.search(r'hardware version(\s)*:(\s)*\w+', result, re.M).group(0).split()[-1]
                        try:
                            mac_address = re.search(r'mac address(\s)+:\s(?:[0-9a-fA-F]:?){12}',result , re.M).group(0).split()[3]
                        except:
